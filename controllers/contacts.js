...
const createContact = async (req, res) => {
    ...
    if (result.acknowledged) {
        ...
        res.status(500).json( response.error || 'Failed to create contact' );
    }
};
...
const deleteContact = async (req, res) => {
    // #swagger.tags = ['Contacts']
    const contactId = new ObjectId(req.params.id);
    const result = await mongodb.getDatabase().db().collection('contacts').deleteOne({ _id: contactId }, true);
    if (result.deletedCount > 0) {
        res.status(204).send();
    } else {
        res.status(500).json(response.error || 'Failed to delete contact');
    }
};
